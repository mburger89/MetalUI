...
and the height it keeps is the one line it had before shrinking, so glyphs will
spill out of the box once anything draws them. Pinned by
...
| `Text.foregroundColor` / every glyph / **the whole glyph draw path** | **Stored, and read by no production code — a `Text` occupies the right box and draws nothing inside it.** `Text.paint` emits its background if one is set and returns. **The gap is no longer "the renderer's text pipeline does not exist" — it does, as of Task 7, and the missing piece is the one thing that would join the two halves.** `MUIGlyph`, `Scene.glyphs`, `Scene.insert(_: MUIGlyph)`, `glyph_vertex`/`glyph_fragment` and `Renderer.upload(_:)` are all implemented and tested against the GPU; `GlyphAtlas`, `GlyphImage.left`/`.top` and `GlyphRaster.subpixelPlacement(forDeviceX:)` are implemented and tested on the CPU. **Nothing in `Sources/` calls `Renderer.upload`, `Scene.insert(_: MUIGlyph)` or `GlyphAtlas(width:height:)`** — re-measure with `grep -rn "\.upload(\|GlyphAtlas(" Sources/`, which returns **nothing at all** — the patterns are call shapes, so a declaration does not match one and there are no calls. What is missing is the **emitter**: something that owns a `GlyphAtlas` on the window beside the `ShapingCache`, sequences `beginFrame`/`endFrame` around the paint pass, walks a `ShapedText`'s runs turning each glyph's pen position and `GlyphImage` bearings into an `MUIGlyph`, and calls `upload` before `encode`. That is `MetalUI` work — `Frame`, `Passes`, `Window`, `Text.paint` — and Task 7's brief scoped it out and Task 8's does not ask for it. The modifier exists rather than being withheld (unlike the four `Style` properties above) because the value it stores is the one the emitter will consume unchanged. **Delete this row in the task that draws a glyph.** The row this replaces said `MeasureFunction` / `tree.measure()` had **no production caller**: `Text` is that caller, through `LayoutPass.requestLeaf` → `Frame.requestLeaf` → `newLeaf` — re-measure with `grep -rn "newLeaf" Sources/`, which now finds the call in `Frame.swift` among the declaration and its doc comments. §9.2's content branch and §4.5's automatic minimum are live for **leaves** as well as containers, and the leaf halves have browser-verified consequences for the first time — two of them, of which one (an `auto` cross size measured as max-content on a column's inline axis) was **fixed** in its own task and the other is divergence 6 above |
