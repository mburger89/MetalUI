...
(**443 today**; the count is quoted so the measurement can be dated, not because
...
wrong scale) and rules out none of §4.2's three.
...
| `GlyphAtlas.evictUnusedSince(_:)` | **Zero production callers, and that is a decision with a mechanism behind it rather than an omission.** `grep -rn "evictUnusedSince" Sources/` finds the declaration and nothing else. The frame brackets it depends on *are* sequenced — `Frame.render` calls `beginFrame`/`endFrame` around the paint phase — so the ordering guard it exists for is live and enforceable; what nothing does is call it. **Calling it every frame today would make text worse, not better**: the shelf packer never revisits a closed shelf, so an evicted glyph's pixels stay resident and unreachable and the next frame that wants it packs a *second* copy further down. Eviction is a net gain only once something reclaims the space — a repacker, or a whole-atlas rebuild — and neither is M2's. The consequence to know: the atlas is a **grow-only** cache, and a window showing an unbounded stream of distinct glyphs fills it, after which `Frame.draw` silently drops the ones that do not fit. Its guards (`evictingDuringFrameConstructionTraps`, `aGlyphUnusedSinceAnOlderGenerationIsEvicted`) stay for `LayoutTree.reset`'s reason: they pin the contract for whoever does call it |
...
`swift build` · `swift test` — **443 tests** and 67 browser fixtures, warning-free
(measured 2026-08-27 `--no-parallel` on `feat/text-m2` after the glyph emitter,
which added fourteen tests and rewrote one; read the
