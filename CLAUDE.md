...
| `Element.elementID` / `ElementID` / `GlobalElementID` | **Declared, never read.** `Frame.render` passes `nil` as the `GlobalElementID` to all three phases, and no code reads an element's `elementID`. The mechanism that would consume them — the side table keyed by the identity *path*, marked on access and swept per frame (§4.3) — is not written. Until it is, two elements with the same local id are indistinguishable, and nothing carries across a frame |
...
| `AnyElement` / `ElementObject` / `AnyElementBox` | **Fully implemented, zero production callers.** The erasure works — it is not a stand-in — but nothing in `Sources/` constructs an `AnyElement`: `Frame.render` is generic over a concrete `E: Element`, and the containers that would hold `[AnyElement]` are Task 4's. Every property it has is therefore exercised only by tests. **It must not become the default path when they land** (§4.6 allocation mitigation 1): a result builder preserves concrete types, so `Column { Label(…); Button(…) }` builds `Column<Pair<Label, Button>>` and boxes nothing; this file is for genuinely dynamic children. Delete this row when a container uses it *and* the static path demonstrably does not |
...
`swift build` · `swift test` — 220 tests and 57 browser fixtures, warning-free.
