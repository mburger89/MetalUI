    ...
    public static func shape(_ string: String, font: ResolvedFont,
        ...
        if let width, attributed.length > 0 {
            ...
            while start < attributed.length {
                ...
                // later.
                //
                // The mechanism that keeps this unreached today, so a future
                // reader can re-check it rather than trust it: CoreText returns
                // at least one UTF-16 unit for every `start < length`, at every
                // width — measured on this OS across widths 0, -1, -5, 0.0001,
                // 0.5, 1 and 5, at every start index of a mixed
                // Latin/CJK/Arabic string, and for eighteen strings chosen to
                // start on a break-sensitive character (leading newline, CRLF,
                // U+2028, U+2029, ZWSP, NBSP, soft hyphen, U+FFFC, a ZWJ emoji
                // sequence, Thai and Devanagari clusters). It returns 0 for
                // exactly one input — `start == length` — which the loop
                // condition excludes. So **no string and no width this function
                // can be handed reaches this precondition**, and no test in the
                // suite reddens when it is deleted; it is a contract assertion
                // against a future CoreText, not a live guard. See the task-2
