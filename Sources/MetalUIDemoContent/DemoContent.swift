...
// The demo's content: its model, its actions, the counter, `demoContent()` and the
// proposal preview (`nativeLayoutPreviewContent()`). Moved out of
// `Sources/MetalUIDemo/main.swift` into this library target so the tests import
// the tree the demo runs rather than a copy of it (plan task 7, stage 1, lane 5;
// ruling LR-S in `docs/superpowers/2026-09-17-engine-replacement-decisions.md`).
// `main.swift` keeps `runDemo()`: the app, the window, the keymap and the action
// handlers. Every "`runDemo`" below means that function, in that file.
//
// **Module-level state is `@MainActor`** (`demoModel`, `demoRows`, `demoWindow`,
// `counterID`, `didFocusCounter`): top-level code in `main.swift` was main-actor
// isolated implicitly, and a library's globals are not. The access level is
// `public` only where `main.swift` reads a name; the tests reach the rest with
// `@testable import MetalUIDemoContent`. This is demo content, not framework API.

/// Whether the modal is on screen. Toggled by the **M** key (`runDemo` in `main.swift`)
...
///
/// **`@MainActor` on the instance.** Until plan task 7's lane 5 this was
/// top-level code in `main.swift`, where the compiler rejects an explicit global
/// actor ("top-level code variables cannot have a global actor") because it
/// already isolates them to the main actor; in this library target the global
/// is not isolated unless it says so (ruling LR-S).
///
...
public final class DemoModel {
    ...
    /// Toggled by the **A** key (`runDemo` in `main.swift`), inside a real
...
@MainActor let demoRows = (0..<demoRowCount).map(DemoListRow.init)
...
public struct ToggleAnimationDemo: Action { public init() {} }
/// Terminates the app. Bound to **Q**. The reactivity instrument's summary
/// (`printReactivitySummary`, in `runDemo`) prints on **this** quit path and on the
...
public struct QuitDemo: Action { public init() {} }
...
/// The action handlers in `runDemo` capture this global instead of capturing `window`,
/// and this holding it weakly is what keeps the cycle from re-forming one step
/// further out.
///
/// **`@MainActor`** for `demoModel`'s reason: it was top-level code in
/// `main.swift`, implicitly main-actor isolated, until ruling LR-S moved it here.
@MainActor public weak var demoWindow: Window?
...
/// framework.
@MainActor public var counterID: GlobalElementID?
...
@MainActor var didFocusCounter = false
...
struct CounterPanel: Element {
    ...
    static func chrome(count: Int,
        ...
        return Box(style: row,
            ...
            // predicate in `runDemo`. A context is contributed by any element in the
...
private func demoMainPane(heroStack: some Element, scrollBox: some Element) -> some Element {
    Column(gap: Pixels(12)) {
        ...
        // lowered text measurement wraps at it — so the paragraph
        // re-wraps on every horizontal resize with no
