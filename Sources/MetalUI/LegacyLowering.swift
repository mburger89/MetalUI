...
//
// **Lane 2 lowers leaves** — a childless `Box` and a `Text`. A node with children
// still reports `(site, "noLowering")` until lane 3 adds the container half.
...
extension LayoutPass {
    ...
    /// no content size of its own, only its padding and its declared size. A node
    /// with children reports `(site, "noLowering")` — lane 3's container half.
    func lowerLegacyNode(_ style: Style, declared: Style, children: [LayoutNodeID],
                         site: LoweringSite) -> LayoutNodeID {
        guard children.isEmpty else {
            return frame.unlowerable(UnlowerableField(site: site, field: "noLowering"))
        }
