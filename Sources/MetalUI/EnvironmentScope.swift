...
///
/// **The typed entry is a copy of `requestGroupLayout` above, so it is pinned
/// on its own** (rulings EV-W item 1, MC-H): it resolves the write once, pushes
/// the values around `content.requestProposalGroupLayout`, and forwards
/// `parent` and `cursor` unchanged. A bare forward skips the layout-phase push
/// (the layout slot of `proposalContentReadsTheEnvironmentThroughAScopeInEveryPhase`
/// reads 0); a cursor bump or a value-keyed parent reddens
