...
@Test @MainActor
func aWheelEventInsideARegionScrollsIt() throws {
...
@Test @MainActor
func aWheelEventOutsideEveryRegionScrollsNothing() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let (window, platformWindow) = try makeFakeWindow(device: device, size: 120) {
        ScrollView(.vertical, elementID: ElementID("list")) {
            Box(style: columnStyle()) {
...
@Test @MainActor
func theTopmostOverlappingRegionWinsAndTheOtherDoesNotMove() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let (window, platformWindow) = try makeFakeWindow(device: device, size: 120) {
...
@Test @MainActor
func momentumDeltasScrollLikeDirectOnes() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let (window, platformWindow) = try makeFakeWindow(device: device, size: 120) {
        ScrollView(.vertical, elementID: ElementID("list")) {
            Box(style: columnStyle()) {
...
@Test @MainActor
func aNestedRegionClippedOutOfViewByItsParentDoesNotReceiveWheelEvents() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let (window, platformWindow) = try makeFakeWindow(device: device, size: 120) {
        ScrollView(.vertical, elementID: ElementID("outer")) {
            Box(style: columnStyle()) {
...
@Test @MainActor
func aHorizontalScrollViewMovesOnDeltaXNotDeltaY() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let (window, platformWindow) = try makeFakeWindow(device: device, size: 120) {
...
@Test @MainActor
func scrollingPastTheEndDoesNotBankAnOffsetTheUserMustUnwind() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let (window, platformWindow) = try makeFakeWindow(device: device, size: 120) {
        ScrollView(.vertical, elementID: ElementID("list")) {
            Box(style: columnStyle()) {
...
@Test @MainActor
func aDeferredScrollViewTakesTheWheelFromAnOverlappingSiblingBeneathIt() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
...
@Test @MainActor
func withinOneLayerTheLastRegisteredRegionStillWins() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let (window, platformWindow) = try makeFakeWindow(device: device, size: 200) {
        Box(style: stackStyle()) {
            Deferred {
...
/// `width`×`height` (and, until stage 9, a legacy leaf under the legacy
/// authority).
///
...
private struct ScrollContextRecorder: Element, StyledElement {
    ...
    func requestLayout(_ id: GlobalElementID,
        ...
        let node = pass.frame.requestNativeLeaf { _ in LayoutMeasurement(size: SizeD(width: w, height: h)) }
        return (node, node)
    }
...
@Test @MainActor
func rawOffsetPublishedDuringRequestLayoutCanExceedTheClampedRange() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let seen = ScrollContextRecorder.Seen()
    let (window, platformWindow) = try makeFakeWindow(device: device, size: 120) {
        ScrollView(.vertical, elementID: ElementID("list")) {
...
/// from a leak.
@Test @MainActor
func nestedScrollViewsInnermostWinsAndPoppingRestoresTheOuterContext() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
...
/// `nil`.
@Test @MainActor
func aSiblingAfterAScrollViewSeesNoScrollContext() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let seen = ScrollContextRecorder.Seen()
    let (window, _) = try makeFakeWindow(device: device, size: 120) {
        Box(style: columnStyle()) {
...
/// being checked.
@Test @MainActor
func aNonOpaqueDeferredScrimLetsTheWheelReachTheListBeneath() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let (window, platformWindow) = try makeFakeWindow(device: device, size: 200) {
        Box(style: stackStyle()) {
            Deferred {
...
@Test @MainActor
func aNestedScrollViewInsideAScrolledOneReceivesTheWheelWhereItPaints() throws {
    let device = try #require(MTLCreateSystemDefaultDevice())
    let (window, platformWindow) = try makeFakeWindow(device: device, size: 200) {
        ScrollView(.vertical, elementID: ElementID("outer")) {
            Box(style: columnStyle()) {
