...
/// `TombstoneTests` ran under **both** layout authorities — 87 since stage 6b's
/// lane 1, which added the five `hidden()` accessibility tests (`LR-DH`); 82 since
/// stage 5's lane 3. Its lane 2 added seven: `DeferredTests`' four element-level scenarios and
...
/// summary line — measured on this suite, `LR-BI` — so parameterising 87
...
/// This test holds the hand-derived literals: the 87 names, and the `arguments:`
/// list itself.
///
...
/// by here all 87 have run. If that ever changes this fails **naming the scenarios it had not
