...
drops the average depth from ~12 to ~3. The linked list moves 0.154 → 0.144
µs/node (0.093 → 0.084 in release) — **flat**, which is the O(1) claim. The
array form moves 0.697 → 0.501 debug and 0.300 → 0.141 release, so its cost
**does** track depth, and release makes that plain: 2.1× for a ~4× depth
difference, with the rest being the array allocation that a shallow path still
pays.
...
branch is **deleted rather than unreachable**. The optional survives in exactly
the places a root forces it — `child`'s and `init`'s `parent` parameters,
`GlobalElementID.parent`, `requestGroupLayout`'s `parent`, and two locals inside
`==`.
