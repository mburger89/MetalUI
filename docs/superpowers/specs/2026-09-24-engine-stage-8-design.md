...
out at 129 of the 22 files' 425 sites, K at 296** (`LR-FB`: the site-coverage
fallback is per test, over four mutations — Ms1–Ms3 and lane 2's M2b as Ms4,
added by lane 3's review round); the Record phase is next.
...
| `LR-FB` | lane 3: the site-coverage fallback is per test, not per file (spec §6 step 3 amended); measured outcome F 129 / K 296 in the 22 F files; a `Box`'s own background pinned by M2b as a fourth Ms mutation (lane 3 review round), its ten dropouts K |
...
| **N1.4** `aFramedAbsoluteBoxStillReportsEveryOtherFieldAndItsPositionOutsideADeferred` (`PresentationLoweringTests`) | under `.proposal` with diagnostics: `.frame(20×20).position(.absolute).inset(…).flexGrow(1)` in a `Deferred` reports `modifierLayer.style`; `.frame(20×20).position(.absolute).inset(…)` in a `Column` with no `Deferred` reports `modifierLayer.position` then `modifierLayer.inset`; arm 3: a two-member `Component`'s `.frame(width: 20, height: 20).position(.absolute).inset(…)` in a `Deferred` still reports `modifierLayer.style` (`LR-EY` item 3); arm 4 (`LR-FA`): the framed absolute box as the frame's root reports `modifierLayer.position.unconsumed` then `modifierLayer.inset.unconsumed` | arm 2 reports `style` (not `position`) before step 2; arm 4 reports `[]` before `LR-FA` | **M1c**: skip the comparison entirely when absolute → arm 1 green-for-wrong-reason, N1.4 red; **M1d**: restore the `.frameLayer` guard in `planLegacyItems` → arm 2 lowers silently, N1.4 red; **M1h**: drop the one-node condition from the exemption → arm 3, N1.4 red; **M1i**: restore `reportUnconsumedLoweredItems`' unconditional `.frameLayer` skip → arm 4, N1.4 red |
...
(`Stack.paint` skips the same), **Ms4** (lane 2's M2b: `Box.paint` drops its
own background, hover and focus colours; added by lane 3's review round, run
with lane 2's five `--skip`s) and record every reddened test name; (2) convert
...
K fallback per test where an assertion would move; (3) re-run Ms1–Ms4: **each
