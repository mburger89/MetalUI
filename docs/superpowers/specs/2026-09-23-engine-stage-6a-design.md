...
Totals: **R 66 tests** (lane 2: 52, lane 3: 14 — *as landed, lane 2's R elements are laid out by 59 tests, `LR-DC` item 1; lane 3's by 12: `anEmptyComponentContributesNoNodes` and `contentIsMaterializedExactlyOncePerFrame` render no lane-3 element, `LR-DD` item 1 — and by 11 after `LR-DE` moved `aSingleAxisFixedFrameDoesNotPinTheAxisItDidNotDeclare` to P-CSS*); **P 57** — 32 P-CSS (lane 2: 3,
lane 3: 29; *33 and 30 as landed, `LR-DE`*), 23 P-6b (lane 2: 6, lane 3: 17), 2 P-9 (lane 2); **Dual** 6 elements (*5 as landed: `ClickCounter` is P-6b, `LR-DC` item 2*); **L** 10 fixtures; **Dep** 6
