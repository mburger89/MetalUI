...
**Status, 2026-09-23 (PDT): DESIGNED, critic round 1 applied (`LR-DA`); lane 1 landed (`c95dc0d`, record §30 §8, `LR-DB`); lane 2 landed (`e683975`, record §30 §9, `LR-DC`); lane 3 landed and closed the gate (`5822f60`, `ab7d76f`, `e531260`; record §30 §10–§11, `LR-DD`) — 1642 tests, 0 `warning:` on both build systems, the exit test green.** Every measurement
...
| `StateTableTests` | `CountingElement` (175) | R | `aSharedTableCarriesStateAcrossFramesWhileAPerFrameTableWouldNot`, `anElementThatStopsBeingProducedLosesLivenessButKeepsItsValue`, **and eleven `IdentityTests` over it** (`LR-DC` item 3; `childrenOfDistinctUnnamedParentsDoNotCollide` renders nothing) |
...
| `EnvironmentTests` | `ClickCounter` (297) — 20×20 | **P-6b** (*was Dual; `LR-DC` item 2: the R test named here renders `EnvRecorder`*) | `changingADisabledOrEnvironmentValueKeepsTheStateBelowTheWriter` (CE+RP: red in A2, B2, B3; green in C2) |
...
| `PipelineTests` | `ProbeRow` (65: a 400×100 row over 100×40 and 60×20) | R | `theThreePhasesRunInOrder`, `eachFrameOwnsItsOwnStateSoNothingLeaksBetweenFrames`, **CE** `prepaintSeesBoundsTheEngineResolvedBetweenTheFirstTwoPhases`, **CE** `paintReceivesTheRootBoundsAndEmitsIntoTheFramesScene` — re-spelled as **one `ProposalLayout` (`ProbeRowLayout`, a fixed 400×100, children left to right at the top)** over two sized leaves (*as landed, `LR-DC` item 4: a frame over a stack is 4 nodes and `eachFrameOwnsItsOwnStateSoNothingLeaksBetweenFrames` counts 3*; all-native, so no authority argument) |
...
Totals: **R 66 tests** (lane 2: 52, lane 3: 14 — *as landed, lane 2's R elements are laid out by 59 tests, `LR-DC` item 1; lane 3's by 12: `anEmptyComponentContributesNoNodes` and `contentIsMaterializedExactlyOncePerFrame` render no lane-3 element, `LR-DD` item 1*); **P 57** — 32 P-CSS (lane 2: 3,
lane 3: 29), 23 P-6b (lane 2: 6, lane 3: 17), 2 P-9 (lane 2); **Dual** 6 elements (*5 as landed: `ClickCounter` is P-6b, `LR-DC` item 2*); **L** 10 fixtures; **Dep** 6
