...
| `everyNativeRegistrarAcceptsNativeChildrenWithoutTrapping`: exit `.success`; all twelve registrars over native children, with `precondition`s on the node count and on each `style(id) == .default` | green on arrival; its red run needs the checking `newNode`, so it is taken after the implementation (`SA-T` item 4) | route native registrars through the checking `newNode`, which then traps. **The run truncates** (shape 11): every in-process native test with a child traps too, so read this test's red line from the log before the truncation |
...
| `resettingATreeDuringLayoutTraps`: a native closure calls `reset(generation: 1)`; fragment `"reset(generation:) called while layout is running"` | the arrays empty mid-run and the run dies on the next stale-id read (measured: `LayoutNodeID from generation 0 used against a LayoutTree at generation 1`; `SA-T` item 3). The process fails, but stderr lacks the fragment, so the test is red on its second assertion. | delete the `reset` precondition |
| `writingARectDuringNativeMeasurementTraps`: the leaf is the **root**, and its closure calls `setLayout`; fragment `"during native measurement"`. Under a parent the closure runs inside the parent's bracket too, and the mutation stays green (measured, `SA-T` item 2) | exits 0 | move `measureDepth += 1` after the leaf closure call |
...
| `aProposalMarkedElementThatRegistersALegacyNodeTrapsInsideAProposalContainer`: exit test. An `Element, ProposalElementGroup` whose `requestLayout` calls `pass.requestNode`, placed in an `HStack`, followed in a `VStack` by a witness sibling whose `requestLayout` traps with its own message; fragment `"contains a legacy node"`. Without the witness the mutation stays green, because measurement's `nativeNode(_:)` lookup traps later with the same message (measured, `SA-T` item 1) | **green on arrival** | delete the child loop from `newNativeLinearStack` |
