...
| 3.3 | `aPresentationKeepsItsDeclaringScopesEnvironmentUnderBothAuthorities` (theme and `.disabled`, and — lane 3's corrections, `LR-CS` item 1 — a layout-phase reading of a scoped key) | as 3.1 | **M3b** `Deferred.requestLayout` wraps content in the root environment (both) — **measured green against the theme and the gate alone** (both are read in prepaint/paint, where `EnvironmentScope` re-pushes its values), red only through the layout-phase reading; **M3b′** the root environment around all three phases (both; theme, gate and reading) |
| 3.4 | `aPresentationsAccessibilityRecordAndFocusMatchUnderBothAuthorities` — the scrim's "Close modal" button record with window geometry; focus inside the presentation dispatches a key | as 3.1 | **M1c** (the record's geometry; the proposal arm only) |
...
| 3.6 | `nestedPresentationsLandOnOneLayerUnderBothAuthorities` (`AP-H`) — a tooltip presentation inside the demo-shaped `inset(0)` scrim: both on layer 1, no report | as 3.1 | **M3d** `pushLayer` → `activeLayer + rootLayer` (both; also the pass-level `nestedDeferredsAllLandOnTheSameRootLayer`); **M1c** reddens its proposal arm too (`LR-CS` item 3) |
| 3.7–3.8 | the two parameterised existing pins | parameterised before anything else: measured, expected green (in-flow `Deferred`s lower today) | **M3a** (3.7), **M3b′** (3.8) — this row read "M3a, M3b"; measured, M3b as spelled reddens neither (`LR-CS` item 2) |
