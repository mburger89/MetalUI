...
| rects' corner radii, all four corners (verifier round, finding N3) | the padding-4 layer's radius (3) and the outermost's (9) exchanged | the rect list, and equal to the oracle's once radii are zeroed |
| the order of the two INNER layers' fills (verifier round, finding N8) | the oracle's own rect list with entries 1 and 2 (the frame's and padding 4's fills) exchanged | the rect list; the oracle is required to hold exactly 4 rects first |
...
| 4 | `aModifierChainIsIdenticalToHandBuiltNestedBoxes` (`MC-B`) | The chain `leaf.background(.accent).onClick{}.padding(4).id("mid").background(.surfaceSecondary).cornerRadius(3).frame(width: 60, height: 40).background(.surface).cornerRadius(5).onClick{}.padding(8).background(.separator).cornerRadius(9).onClick{}` (the padding-4 background and the three radii added in the verifier round, so TWO inner layers fill, each with a radius unlike the outermost's), in a `Row`, against hand-built `Box(style:decoration:content:)` values, one per wrapper, with the same styles, handlers and `.id`. It compares: the leaf's id and bounds; `lastHitboxes` ids, bounds and order; the scene's rect order, bounds, colour and four corner radii; `tree.nodeCount`; and `isLive(animRetentionSlot(for:))` per layer id. The seven disagreeing oracles in the table above are each `try #require`d first. **It does not compare accessibility records or anything else `Element`'s group defaults do per element** (`MC-B`; the AB-O merge obligation below) | green on arrival | **today:** `FrameModifier.prepaint` registers its handlers after its content; `FrameModifier.requestLayout` skips `animated` (R4, also reddens test 5). **Lane 2**, each run separately: the layer loop runs innermost-first in prepaint; every inner layer takes the outermost id; `.id` lands on the wrong layer; **one inner layer skips `animated`**; **the inner-layer paint loop runs innermost-first** (N8); **an inner layer's fill takes the outermost layer's corner radius** (N3) |
...
| 8 | `aModifierChainRegistersAndPaintsOuterLayersFirst` (`MC-F`) | The chain `leaf.background(.accent).onClick{"inner"}.padding(4).background(.surface).onClick{"outer"}`. Hitbox order is `[outer, leaf]`. `.surface` is emitted before `.accent`. A click in the padding ring logs `outer`; one inside the leaf logs `inner` | green on arrival | **today:** `Box.prepaint` registers after content. **Lane 2:** a layer registering after everything inside it (L4), and a layer's fill emitted after the content (L5). **Corrected in lane 2's verifier round:** this row first read "the layer loop reversed in prepaint, and separately in paint", which this test cannot see — its chain has ONE padding layer. Registration-order reversal is tests 4 and 2's (L3); inner paint-loop reversal is tests 4 and 2's since the verifier round (N8), and before it was seen by no test |
