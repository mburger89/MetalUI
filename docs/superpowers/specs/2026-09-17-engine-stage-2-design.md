...
extension LayoutPass {
    ...
    /// *Lane 5, `LR-BA` item 2, as amended after the lane's verification*: it returns
    /// the stack's **spacing** — a distributed container's drops to 0 — and **not**
    /// the main factor, which the caller needs before this call (the diagnostics
    /// bail-out path registers no node and still records its content alignment) and
    /// so reads from `legacyMainFactor(_:)` itself; a second copy in the tuple had no
    /// reader.
...
| 4.9 | *(added after the lane's verification, which found the read unpinned)* `aLoweredMarginRegistersItsAnimatedValue` — `Row { a20.margin(px(0 → 40)); b20 }` under `withAnimation(.linear(duration: 1))`, one `Frame` per authority with an explicit `transaction:`/`timestamp:` (no window, no ticks): the first child sits at x 0 on the frame that starts the transaction and at x **20** half-way, under the proposal authority exactly as under the legacy one | — (the read landed with `55621aa`; the arm is the pin it lacked) | **V4n** `plan.marginInsets` read from the declared style (`d.margin`) instead of the animated one |
...
`LR-BA` item 5; *after the lane's verification* those five arms go through
`LayoutDifferential.compare` rather than a proposal-only render, so the **agrees**
half of the new name is checked for them too — an unsized, ungrown reverse container
is a shape `LoweringDistributionTests.swift` does not carry) and
