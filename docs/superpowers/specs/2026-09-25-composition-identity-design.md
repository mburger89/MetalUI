...
M2a′ also reddens three typed-primary overlay/background tests. **Amended
again by `ID-P` item 7**: C2.13 (the typed `OptionalGroup` copy's reset,
mutation N1) makes lane 2 **1454 → 1466**.
...
| **B3.4** `aLegacyBackgroundLowersBothSidesAsAFrameLayerDoes` (added by the lane-3 fix round, `ID-Q` item 6) — `Row { Box(30×20).flexGrow(1).background(.topLeading) { Box(10×10).margin(5) }; Box(10×10) }` under diagnostics: empty report, primary (0, 40, 30, 20), background (0, 40, 10, 10), sibling (30, 45, 10, 10); a background-side `Deferred` presents at (5, 5) against the window and the tree holds 7 nodes | green on arrival (the fix round's pin of an implemented line) | **V5** `let primary = contentNodes`; **V6** `let secondary = backgroundNodes` |
...
- **Suite: 1444 → 1452 → 1463 → 1479** (`ID-M`); **1444 → 1454 → 1465 → 1481** after `ID-O`'s two lane-1 tests (`ID-P` item 4); **1444 → 1454 → 1466 → 1482** after C2.13 (`ID-P` item 7). Guards 84 → 84 → 85 → 88.
