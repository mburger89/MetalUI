...
Branch `feat/data-and-scrolling` from `e7bc2e7`. Rulings `DD-A`…`DD-M` in
...
(arms cited as F*, B*, L*, W*, T*, P*, I*, K2) and, from the critic round,
[`docs/probes/swiftui-scrollviewreader-scope.swift`](../../probes/swiftui-scrollviewreader-scope.swift)
...
| ids equal in description, different in value | **both** evaluated (S6) | — (names are `String`s) | **divergence 79, added**: only the first produced, `DD-L` | lane 1 |
...
| a `List` below a header in a `ScrollView` | SwiftUI's `List` there is **0 tall, blank** (L1); the ordinary shape, `LazyVStack`, realises the rows on screen, 0…3 (L2) | builds rows **8…16**, all off screen (DD14) — divergence 14 | **fix** to L2's answer, 14 retires, `DD-F` | lane 2 |
| a grown viewport with no input | — (internal) | stale window **persists**, no frame drawn (DD13) — divergence 13's effect | **fix**: one more frame when stale; 13 amended, kept, `DD-F` | lane 2 |
...
| `ProposalScrollView` publishing a `ScrollContext` | — | none (`LR-BF`) | joins the prepaint scroller stack; layout context still unread → lazy stacks, `DD-I` | lane 2 (pinned by lane 3's 3.12) |
...
| unrealised rows | LazyVStack 4500 (T10); a `List` row reachable (T15, 3610 — SwiftUI's rows are not 30 tall) | — | match (`List` by index at its uniform `rowHeight`) | lane 3 |
| a two-member `ForEach` element | first member (T12/T12b: 330) | — | match | lane 3 |
| any `.id` view | target (T13) | — | match | lane 3 |
| a proxy's reach | its reader's subtree only (S2: key under another reader → nothing moves) | — | match, `DD-G` item 1 | lane 3 |
| key comparison | by value: `10` ≠ `"10"` (S3–S5) | — | match, `DD-K` (the design's description rule superseded) | lane 3 |
...
Run in the order 1, 2, 3 (lane 3's 3.10 needs lane 1's `ForEach`; lane 3
builds on lane 2's `ScrollerFrame` stack and edits `List.swift`, `Frame.swift`
and `ScrollView.swift` after it — `DD-A`'s critic-round amendment). Each lane:
...
| 1.8 | `aForEachDuplicateIDProducesOnlyTheFirstElement` | [x, x]: one node registered, the first element's content only (F7) | **M1e**: the per-frame dedupe removed (both halves) |
...
| 1.16 | `aForEachWhoseIDsCollideInDescriptionProducesOnlyTheFirst` (divergence 79's pin, `DD-L`) | ids `AnyHashable(1)`, `AnyHashable("1")`: one node, the first element's content (SwiftUI: both, S6) | **M1l**: the dedupe's name half removed — reddens 1.16 alone (1.8 stays green on the value half) |
...
**And, since the critic round (`DD-A` amendment), `DD-F`:**
`Sources/MetalUI/List.swift` (stored origin, fresh-window check),
`Sources/MetalUI/ScrollView.swift` and `Sources/MetalUI/ProposalScrollView.swift`
(push a `ScrollerFrame` — the `ProposalScrollView` push has no reader until
lane 3, whose 3.12/M3l is its pin; lane 2 says so in its report rather than
claiming it pinned), `Sources/MetalUI/Passes.swift` (`deferred` resets the
scroller stack), `Sources/MetalUI/Frame.swift` (the scroller stack). Tests:
`Tests/MetalUITests/ListTests.swift` (the divergence-14 pin retired and
replaced by 3.1).
...
**`DD-F`'s five tests, run by lane 2** (ids and mutation names kept from the
design so citations hold; the origin goes through `withState`, never `write` —
`DD-F` item 1's critic-round note, pinned by 3.4):
...
Unchanged and required green after lane 2: `aListInsideADeferredIgnoresTheEscapedScrollersOffset`,
...
the 100 000-row test (`METALUI_RUN_100K_LIST_TEST=1`, run once by the lane —
it reads lane 2's `List` change) and the frame-loop pause tests.
...
(`DD-D` item 6); its fact is inverted by G2.1.
...
`ScrollView`. **Guard delta**: −1 (`EnvironmentCompileGuards` 8 → 7) + 3
(`BindingCompileGuards`).

### Lane 3 — the scroll APIs and indicators (`DD-G`…`DD-I`, `DD-K`)

**Files** (on top of lane 2's commit). `Sources/MetalUI/List.swift`
(pending-request scan by value, typed row keys — `DD-K`),
`Sources/MetalUI/ForEach.swift` (typed element keys while a request is pending
— `DD-K`), `Sources/MetalUI/ScrollView.swift` (the enum's two cases),
`Sources/MetalUI/ScrollChrome.swift` (`.never`/`.visible` in `paintIndicator`),
`Sources/MetalUI/Frame.swift` (`recordElementBounds`' request match, the typed
key map, the post-prepaint resolution), `Sources/MetalUI/Window.swift`
...
(new). Tests: `Tests/MetalUITests/ScrollToTests.swift` (new),
...
| 3.13 | `scrollToIsScopedToItsReader` | (S1) two readers, each over a scroller with an element `.id("x")`: reader A's proxy moves only A's scroller; **(S2, the separating arm)** the key only under reader B: A's proxy moves **neither** | **M3m**: the scope check dropped from the match — must redden the S2 arm (B moves to 250); the S1 arm alone cannot see it |
...
| 3.16 | `scrollToComparesKeysByValue` (`DD-K`) | `.id("10")`: `scrollTo("10")` 250, `scrollTo(10)` 0 (S3); a `ForEach(0..<20)` key: `scrollTo(0)` moves, `scrollTo("0")` does not (S5); `List` rows by `datum.id` value; and with **no** request pending, a frame of a 1000-element `ForEach` notes no typed key (the map's count, a work counter, literal 0) | **M3p**: match by `String(describing:)` (reddens the negative arms); **M3q**: typed keys noted unconditionally (reddens the counter arm) |
...
Unchanged and required green: everything lane 2's list names, again (the
100 000-row test re-run once — lane 3 adds `List`'s pending-request scan),
and `aDisabledScrollViewStillScrollsOnTheWheel`.
...
**Retirement rows**: none (the divergence-14 pin retires in lane 2).
**Guard delta**: +2 (`ScrollCompileGuards` 2).
...
By construction (re-derived by the critic round): lane 1 adds **15 tests**
(1.1–1.10, 1.12–1.16; 1.11 is an existing test whose answer changes) and **2
guards**; lane 2 adds **15 tests** (2.1–2.10 and `DD-F`'s 3.1–3.5) and **3
guards**, deletes **1 guard** and retires **1 test**; lane 3 adds **11 tests**
(3.6–3.16) and **2 guards**. A guard is a `@Test`, so the summary line moves by
**+17, +16 and +13**: **1506 → 1523 → 1539 → 1552**. Guards by file:
...
- Record §04: a dated section — **14 and 74 retire, 13 amended, 78 and 79
  added** (78: `Binding` is main-actor isolated; 79: `ForEach` ids colliding in
  description produce only the first, `DD-L`).
...
- `CLAUDE.md`/`AGENTS.md`: the `DD-` prefix (next `DD-N`) in the prefix list;
