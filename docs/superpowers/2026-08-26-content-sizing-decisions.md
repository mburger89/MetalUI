...
- ~~**`measureNode` cannot tell `.minContent` from `.maxContent` for a
  ...
  `IntrinsicModeTests.swift` guards the two independently — restoring the
  budget reddens only the wrap test, restoring the `.maxContent` literal only
  the propagation test. Design §5.2's "`.minContent` and `.maxContent` swapped
  ...
  pixel-sized empty div for which the two modes coincide. That half of the risk
  stands and belongs to the re-baseline.
