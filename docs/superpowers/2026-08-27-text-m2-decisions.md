...
**Ruling IDs here are prefixed `TX-` and are LETTERED, `TX-A`…`TX-K`.** A bare
...
| TX-G | **M6's hand-rolled UAX #14 subset is demoted from planned work to a contingency, and M6 should start by trying to delete it.** §6.4's "CoreText exposes no width-independent line-break-opportunity API" is true of CoreText and materially misleading: CoreFoundation has one, and that sentence was the sole justification for the subset. Measured against §6.4's own class list, `CFStringTokenizer` covers every class enumerated **plus Thai `SA`**, which the subset explicitly excludes; its ranges partition the string contiguously; it costs 93.6 ns/char and is cacheable *because* it is width-independent. **(Corrected 2026-09-10: that figure is not a throughput — creation is a flat ~18.4 µs regardless of length and the remainder is ~140 ns/char, so whole-function ns/char runs from 21,708 at one character to 156 at a thousand. The ruling is unaffected; the number was load-bearing only as 'cheap enough', and it still is. See spec §6.4's correction block.)** **Revised 2026-09-10 (lane tokenizer-hoist, commit f2afa55), acting on that correction.** As it says, the recorded "93.6 ns/char, ~22 µs per 231-character line" divides a fixed per-call cost by the string length: `CFStringTokenizerCreate` is a fixed setup cost independent of the string. A replica timing under load (contended; not a recorded figure, so the correction's uncontended ~18.4 µs and ~140 ns/char stay the recorded ones) put the per-call function at ~28 µs on a 37-character row and ~108 µs at 580 characters, and a re-pointed tokenizer at ~7.5 µs and ~90 µs: roughly 150 ns/char plus a small fixed part once creation is removed. `ShapingCache.minContentWidth` now re-points one `@MainActor` tokenizer (`Shaper.unbreakableRunsReusingTokenizer(of:)`). The public nonisolated `Shaper.unbreakableRuns(of:)` still creates one per call, because it is called off the main actor. Pinned as a count: `aMinContentMissReusesOneTokenizerRatherThanCreatingOnePerString`, `aColdFrameCreatesAtMostOneLineBreakTokenizer`. | A milestone's worth of Unicode work is written to replace a system API that already does the job better, and the replacement inherits a correctness surface (`SA`, tailoring, future Unicode versions) the system API maintains for free. |
...
| TX-K | **`.maxContent` is one line per HARD line break, not one line: spec §3.4's "one line, full advance" and the shaper's "one line, no wrapping" were wrong, and both called the defect intended.** `Shaper.shape(wrappingAt: nil)` built a single `CTLine` with `CTLineCreateWithAttributedString`, which lays every hard break's segments side by side, while the wrapping branch's `CTTypesetterSuggestLineBreak` breaks after each one. So max-content was the SUM of a label's lines where every definite width gave the widest: `"Ready\nSet\nGo"` at 13pt measured 75.004 wide and one line tall, against 37.565 wide and three lines. In a centring `Column` 400 wide the box took 76. Its height was already three lines, because the engine asks at a definite width, so the box was centred and a box-centre assertion was green. `Text.paint` re-wraps at the measured width from the box's left edge, which drew the ink left of centre. **Fixed by running the same loop at width `+infinity`.** For break-free text that is byte-identical to the whole-string line: glyphs, positions, advances, string indices, run status and typographic bounds, over bidi, CJK, clusters and a 136,000-unit string. **A finite "large" width is not equivalent.** That string soft-breaks at `1e4` (1,564 units) and at `1e5` (15,912), so the round-2 review's "byte-identical at 1e4" was true only of its short samples. **The separator set is CoreText's and wider than the finding named:** U+000A, U+000D, CR LF, U+2028, U+2029, U+0085, U+000B and U+000C. A trailing break opens no empty last line; that is the typesetter's answer and what the wrapping branch always gave, and TextKit and SwiftUI were not measured. Min-content is untouched, because tokenizer runs never hold an interior separator. Three tests are red on arrival: `aHardLineBreakEndsALineWhenNoWidthIsOffered`, `aLeadingTrailingOrDoubledHardBreakIsOneLinePerBreakUnwrapped` and `aLabelWithHardBreaksMeasuresItsWidestLineAtMaxContent`, the last asserting the widest line's centre rather than the box's. `anUnwrappedBreakFreeStringIsTheLineCoreTextBuildsWhole` guards what the fix must not change; mutating to `width ?? 1e5` reddens it alone. No text fixture was added (TX-B). | Any label containing a newline is laid out at the sum of its lines' widths: it overflows a `Row`, sits off-centre in a `Column`, and its `.background()` paints the inflated rect. Meanwhile a spec row that states this as intended tells the next reader not to look. Conversely, if a future CoreText broke the `+infinity` equivalence, every unwrapped label would shape differently from before; the byte-identity test is what would notice. |
...
included. *(Erratum 2026-09-10: it identifies the font for glyph identity, not its shaping behaviour — the UI font and `"System Font"` at one size share a key and shape non-Latin text differently; `twoRequestsWithEqualFontKeysShareOneShapeThoughTheyShapeDifferently`, pinned wrong on purpose.)* §6.1 measured the trap: requesting `"SFMono-Regular"` by name on the
