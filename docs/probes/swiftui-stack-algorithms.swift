...
// - Spacer(): minLength nil means 8 (SP1 48, SP6); NO default spacing is
//   inserted next to a spacer (SP2 48 = SP1, SP3 40), but EXPLICIT spacing is
//   (SP4 80). A spacer answers 0 on its stack's cross axis (SPB1 100x0, SPB2
//   8x0, SPB5 0x50, X8's overlay proposed 180x0) and both axes outside a
//   linear stack (SPB3 100x50, SPB4 8x8, SPB6).
...
//   priority and its flexibility (SP19 size 50); padding hides only the
//   priority (SP20: still flexible, served last among priority 0); an overlay
//   hides neither (X8). A greedy non-spacer child takes surplus (G3 180, G4
...
//   is the union (A4 60x40). ZStack proposes its proposal to every child and
...
//   X9 HStack(0){a 20; Spacer()} at 100x50 then its reported size at nil @nilxnil: size 28x20
//       leaf a: proposed [nilxnil, nilx20] at (0, 0) 20x20 calls 2
...
@MainActor func measure<V: View>(_ proposal: ProposedViewSize, @ViewBuilder _ view: () -> V) -> CGSize {
    proposalsSeen = [:]; placed = [:]; order = []; calls = [:]; geometry = [:]
...
@MainActor func probes() {
    ...
    run("X9 HStack(0){a 20; Spacer()} at 100x50 then its reported size at nil", none) {
