# 48 — `TextEditor`: multi-line editing, 2026-09-23

Branch `feat/text-editor`, from `master` `6e01d9e`. Ruling `TI-H`, added to
...
- **Under the legacy authority** the editor measures 3 lines high and takes
  its natural width, then is stretched by `Box`. Under the proposal authority
  it is greedy, taking 200 × 200 in a 200-point window.
- **Linux aarch64:** the root's portable suites read 6 + 486 + 3 + 22 and
  `Backends/SDL` reads 21 + 18.

## Counts

1770 tests, 97 goldens, 78 guards; 0 `error:` and 0 `warning:` on both build
systems after `swift package clean` (`Test run with 1770 tests in 3 suites
passed`; the guards ran). **1770 = 1758 + 12**:
- `TextEditingTests` +4;
- `TextEditorTests` +7 (six tests, one of them parameterized over both
  authorities);
