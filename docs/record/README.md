...
| `24-engine-replacement-stage-3.md` | plan task 7 stage 3 on `feat/engine-stage-3`: `ScrollView` lowered onto the kernel scroll viewport, one shared `ScrollChrome` for both scrollers, `Component` amend and wrap as per-member native frames, a frame layer over several members as a row, and both scroll suites running under both layout authorities (the exit test); five lanes, red runs, verifier verdicts (all `ok`, thirteen minors), mutation tables including the verifiers' twenty, the twelve offscreen images and the independently rebuilt harness; counts 1572 / 97 / 77 |
