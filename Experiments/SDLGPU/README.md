...
- `../../Backends/SDL` (**moved there from `Portable/` by roadmap item 7**,
  ruling RS-D, which added the production `SDLWindowRenderer` beside the
  replay tools) — its own package whose MetalUI dependencies are the
...
Portable shaders (one HLSL source, `Backends/SDL/Shaders/replay.hlsl`) need the compiled
stages first: `python3 ../../Backends/SDL/scripts/compile-shaders.py --shadercross <path>` writes
SPIR-V, MSL, DXIL and reflection JSON to `Backends/SDL/Shaders/compiled/`. Then:
...
`Backends/SDL/Shaders/compiled/` matches `replay.hlsl` (`SOURCE.sha256`), records
